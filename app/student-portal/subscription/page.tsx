...
export default function StudentSubscriptionPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch('/api/student-portal/subscriptions');
  ...
  return (
    <div className="space-y-8">
      <HeroHeader
        title="اشتراكي المالي 💳"
        badge="بوابة الطالب"
        subtitle="إدارة المدفوعات والاشتراكات الشهرية"
        stats={[
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="glass-panel p-6 rounded-3xl border border-white/10 shadow-lg relative overflow-hidden">
          ...
          {sub ? (
            ...
            <div className="text-center py-6 text-slate-400">لا يوجد اشتراك نشط حالياً.</div>
          )}
        </div>
        ...
        <div className="glass-panel p-6 rounded-3xl border border-white/10 shadow-lg relative overflow-hidden flex flex-col justify-center text-center">
          ...
          <h3 className="text-lg font-bold text-white mb-2">طلب تجديد اشتراك (قريباً)</h3>
          <p className="text-sm text-slate-400 mb-6">سيتم تفعيل خدمة طلب التجديد والدفع الإلكتروني قريباً.</p>
