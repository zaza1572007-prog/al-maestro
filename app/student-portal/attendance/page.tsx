...
export default function StudentAttendancePage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch('/api/student-portal/attendance');
  ...
  if (loading) {
    return <div className="text-center py-20 text-white animate-pulse">جارٍ تحميل بيانات الحضور...</div>;
  }
  ...
  return (
    <div className="space-y-8">
      <HeroHeader
        title="سجل حضوري 📅"
        badge="بوابة الطالب"
        subtitle="متابعة انضباطك في الحصص التعليمية"
        stats={[
          { label: "نسبة الحضور", value: attendanceRate, color: "text-emerald-400" },
        ]}
      />
