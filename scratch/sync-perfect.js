...
async function syncPerfect() {
  ...
  console.log('⚡ [Perfect Sync] Connecting...');
  ...
  for (const p of srcParents) {
    try {
      ...
      await target.query(`
        ...
        VALUES ($1, $2, $3, $4, $5, $6)
        ON CONFLICT DO NOTHING
      `, [p.id, p.name, `010${Math.floor(10000000 + Math.random() * 90000000)}`, p.relation, new Date(), new Date()]);
    }
  }
  ...
  for (const s of srcStudents) {
    try {
      ...
      console.warn(`  ⚠️ Student warning (${s.name}):`, e.message);
    }
  }
  ...
  for (const sess of srcSessions) {
    try {
      ...
      console.warn(`  ⚠️ Session warning:`, e.message);
    }
  }
  console.log('✅ LessonSessions sync complete.');

  // Refresh valid Student IDs and Session IDs in Target
  ...
  for (let i = 0; i < srcAtt.length; i++) {
    ...
    if (validStudentIds.has(att.studentId) && validSessionIds.has(att.sessionId)) {
      try {
        ...
        console.warn(`  ⚠️ Attendance warning (${att.id}):`, e.message);
      }
    }
    if ((i + 1) % 50 === 0 || i === srcAtt.length - 1) {
      console.log(`  Processed ${i + 1}/${srcAtt.length} attendance records...`);
    }
  }
